// Modified pressure sensor sketch for HouseAgent. This sketch measures pressure from a pressure plug.
// As well as Lux from a lux plug.
//
...
void setup() {
    Serial.begin(57600);
    Serial.print("\n[bmp085demo]");
    rf12_initialize(3, RF12_868MHZ, 1); // 868 Mhz, net group 1, node 3
...
void loop() {
    ...
    struct { int8_t type; int16_t temp; int32_t lux; int32_t pres; } payload;
    //struct { int32_t pres; } payload;
    psensor.calculate(payload.temp, payload.pres);
    //psensor.calculate(temp, payload.pres);

    lsensor.getData();
    payload.lux = lsensor.calcLux();
